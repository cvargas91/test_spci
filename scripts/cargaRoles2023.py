'''
Carga roles de Soporte, Consulta y Información según POA2023 previamente
pasadas a CSV
...
def run():
    print("INICIO DE SCRIPT CARGA_ROLES_2023")
    ...
    with open(rutaRoles, encoding='cp1252') as csvfile :
        ...
        for fila in lector:            
            ...
            if numFila == 0 :                
                ...
                for i in soportes :
                    #input("Press enter to continue ...")
                    if fila[i-1].strip().lower() not in escapes :
                        #sigla = hackSigla(fila[i].strip().upper())
                        sigla = hackSigla(fila[i-1].strip().upper())
